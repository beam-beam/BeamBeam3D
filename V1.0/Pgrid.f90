...
module Pgrid2dclass
  ...
  type Pgrid2d
     ...
     integer comm_beam    ! communicator for my beam only
     ...
     integer npbeam       ! number of processors in my beam communicator
     integer myrank       ! my rank in the grid comm
     integer my_beamrank  ! my rank in the beam communicator
...
contains
  ...
  subroutine construct_Pgrid2d(this,comm,nrow,ncol)
    ...
    ! set up beam communicator
    if (this%my_col < ncol/2) then
       mybeam = 1
    else
       mybeam = 2
    endif

    call MPI_COMM_SPLIT(comm, mybeam, this%my_row, this%comm_beam, ierr)
    call MPI_COMM_SIZE(this%comm_beam, this%npbeam, ierr)
    call MPI_COMM_RANK(this%comm_beam, this%my_beamrank, ierr)
  ...
  ! get my beams communicator.  Split out from getcomm_Pgrid2d
  ! so as not to cause trouble for existing uses of getcomm_Pgrid2d
  subroutine getcommbeam_Pgrid2d(this, commbeam)
    implicit none
    type (Pgrid2d),intent(in) :: this
    integer,intent(out) ::commbeam

    commbeam = this%comm_beam
  end subroutine getcommbeam_Pgrid2d
  ...
  !get my rank position in commbeam communicator.
  subroutine getpost_commbeam(this,mypos)
    implicit none
    type (Pgrid2d),intent(in) :: this
    integer,intent(out) :: mypos

    mypos = this%my_beamrank

  end subroutine getpost_commbeam



  !get the size of the commbeam communicator
  subroutine getsize_commbeam(this,totsize)
    implicit none
    type (Pgrid2d),intent(in) :: this
  ...
  end subroutine getsize_commbeam
