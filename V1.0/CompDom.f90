...
! CompDomclass: 3D global and local computational domain class.
! Version: 2.6
! Author: Ji Qiang 
...
module CompDomclass
  ...
  interface balance_CompDom
     module procedure balance1_CompDom, balance2_CompDom
  end interface
...
contains
  ...
  subroutine init_CompDom(this,distparam,nparam, &
    ...
    integer :: myid,myidx,myidy,comm2d, &
         commcol, commrow, totnp,npxx,npyy
    ...
    double precision :: pi,cons1,cons2,xmin,&
         xmax,ymin,ymax, &
    ...
    double precision, dimension(2,0:nproccol/2-1) :: lctabrgy
    integer :: ip,numint
    integer :: i0,j0,iflag,nphalf,npbc
    !    integer :: ierr
    ...
    elseif(flg.eq.2) then  !6D Gaussian
       xmin = xmu1 -5.0*sig11x/sqrt(1.0-ux*ux)
       xmax = xmu1 + 5.0*sig11x/sqrt(1.0-ux*ux)
       ymin = xmu3 -5.0*sig11y/sqrt(1.0-uy*uy)
       ymax = xmu3 + 5.0*sig11y/sqrt(1.0-uy*uy)
       zmin = xmu5 -5.0*sig11z/sqrt(1.0-uz*uz)
       zmax = xmu5 + 5.0*sig11z/sqrt(1.0-uz*uz)
    ...
    this%Meshsize(1) = (xmax-xmin)/dble(nx-1)
    this%Meshsize(2) = (ymax-ymin)/dble(ny-1)
    this%Meshsize(3) = (zmax-zmin)/dble(nz-1)
    ...
    allocate(this%LcTabrg(4,0:nprocrow-1,0:nphalf-1))
    allocate(this%LcTabnm(2,0:nprocrow-1,0:nphalf-1))

    deltaz = (zmax-zmin)/nprocrow
    deltay = (ymax-ymin)/nphalf
    hz = this%Meshsize(3)
    hy = this%Meshsize(2)
    ...
    if(nprocrow.gt.1) then
       if(flg.eq.2) then
          ...
          interval0 = sumint/dble(nprocrow)
          interval1 = interval0
          sumint = 0.0
          do i=1, numint-1
             tmp = zmin + (i-1)*deltaint
    ...
    if(nphalf.gt.1) then
       ...
       do i = 0, nprocrow - 1
          ...
          enddo
          this%LcTabnm(2,i,nphalf-1) = ny - nsum
       enddo
    endif
    ...
    this%Sptrnglocal(5) = this%LcTabrg(1,myidx,myidy-npbc)
    this%Sptrnglocal(6) = this%LcTabrg(2,myidx,myidy-npbc)
    this%Sptrnglocal(3) = this%LcTabrg(3,myidx,myidy-npbc)
    this%Sptrnglocal(4) = this%LcTabrg(4,myidx,myidy-npbc)
    this%Sptrnglocal(1) = this%SpatRange(1)
    this%Sptrnglocal(2) = this%SpatRange(2)

    this%Mshlocal(3) = this%LcTabnm(1,myidx,myidy-npbc)
    this%Mshlocal(2) = this%LcTabnm(2,myidx,myidy-npbc)
  ...
  end subroutine balance1_CompDom



  subroutine balance2_CompDom(source,lctabnmz,&
       lctabrgz,npz,npy,commrow,commcol,innx,inny,innz,inyglb,&
       inzglb,hz,zmin)
    implicit none
    include 'mpif.h'
    integer, parameter :: Ndiv = 20
    double precision, dimension(:,:,:) :: source
    double precision, intent(in) :: hz,zmin
    integer, intent(in) :: npz,npy,commrow,commcol
    integer, dimension(0:npz-1), intent(inout) :: lctabnmz
    double precision, dimension(2,0:npz-1), intent(out) :: lctabrgz
    integer, dimension(0:npz-1) :: ztable,zdisp
    ...
    integer :: icount,ip,sumcount
    double precision :: interval0,interval1,sumint
    integer :: i0, iflag,jadd,kadd
    double precision :: hnew1,hnew2
    !    integer, dimension(npz) :: nozeronmz
    !    integer :: nozero, myranktest
    !    double precision:: upper, temp
    ...
    !call MPI_COMM_RANK(commrow,myranktest,ierr)
  ...
  end subroutine balance2_CompDom



  ! find the balanced local domain geometry so that the number 
  ! of particles on this domain about equal.
  subroutine balancey_CompDom(source,lctabnmy,lctabrgy,npy,commcol,&
       innx,inny,inyglb,hy,ymin)
    implicit none
    include 'mpif.h'
    integer, parameter :: Ndiv = 10
    double precision, dimension(:,:) :: source
    double precision, intent(in) :: hy,ymin
    integer, intent(in) :: npy,commcol
    integer, dimension(0:npy-1), intent(inout) :: lctabnmy
    double precision, dimension(2,0:npy-1), intent(out) :: lctabrgy
    integer, dimension(0:npy-1) :: ytable,ydisp
    double precision, dimension(inny) :: rhoy
    double precision, dimension(inyglb) :: recvrhoy
    double precision, dimension(Ndiv*inyglb) :: rhoynew
    integer :: i,j,inyglb,innx,inny,ierr
    integer :: icount,ip,sumcount
    double precision :: interval0,interval1,sumint
    integer, dimension(npy) :: nozeronmy
    integer :: i0, nozero,iflag,jadd
    double precision :: upper,hnew1,hnew2
    ...
    if(npy.gt.1) then
       ...
       call MPI_ALLGATHERV(rhoy,inny,MPI_DOUBLE_PRECISION,recvrhoy,&
            ytable,ydisp,MPI_DOUBLE_PRECISION,commcol,ierr)
    ...
    if(minval(lctabnmy).lt.0 .or. maxval(lctabnmy).gt.inyglb) &
         then
       print*,"wrong in the y number of grid in load balance!",&
            minval(lctabnmy),maxval(lctabnmy)
    endif

  end subroutine balancey_CompDom



  subroutine setlctaby_CompDom(this,lctabnmy,&
       lctabrgy,npy,myidy)
    implicit none
    include 'mpif.h'
    type (CompDom), intent(out) :: this
    integer, intent(in) :: npy,myidy
    integer, dimension(0:npy-1), intent(in) :: lctabnmy
    double precision, dimension(2,0:npy-1), intent(in) :: lctabrgy
    integer :: j

    do j = 0, npy-1
       this%LcTabrg(3,0,j) = lctabrgy(1,j)
       this%LcTabrg(4,0,j) = lctabrgy(2,j)
       this%LcTabnm(2,0,j) = lctabnmy(j)
    enddo

    this%Sptrnglocal(3) = this%LcTabrg(3,0,myidy)
    this%Sptrnglocal(4) = this%LcTabrg(4,0,myidy)

    this%Mshlocal(2) = this%LcTabnm(2,0,myidy)

  end subroutine setlctaby_CompDom
  ...
  !--------------------------------------------------------------------------
  !update 2d geometry parameters using new particle positions.
  subroutine update2dold_CompDom(this,ptrange1,ptrange2,grid2d)
    implicit none
    include 'mpif.h'
    type (CompDom), intent(inout) :: this
    type (Pgrid2d), intent(in) :: grid2d
    double precision, dimension(4), intent(in) :: ptrange1,ptrange2
    double precision, dimension(2) :: localrange1, &
         localrange2,temp1,temp2
    integer :: i,j,comm2d,commrow,commcol,nproc,nproccol,nprocrow,&
         myid,myidx,myidy
    integer :: ierr
    double precision :: xmin,xmax,ymin,ymax,hx,hy,deltay,&
         epsx,epsy
    double precision :: t0
    ...
    do i = 1, 2
       if(ptrange1(2*i-1).lt.ptrange2(2*i-1)) then
          localrange1(i) = ptrange1(2*i-1)
       else
          localrange1(i) = ptrange2(2*i-1)
       endif
       if(ptrange1(2*i).gt.ptrange2(2*i)) then
          localrange2(i) = ptrange1(2*i)
       else
          localrange2(i) = ptrange2(2*i)
       endif
    enddo
    ...
    do i = 0, nprocrow-1
       ...
       do j = 1, nproccol-1
          this%LcTabrg(3,i,j) = this%LcTabrg(4,i,j-1)
          this%LcTabrg(4,i,j) = this%LcTabrg(3,i,j) + deltay
       enddo
    enddo
    ...
    do i = 0, nprocrow-1
       do j = 0, nproccol-1
          this%LcTabnm(2,i,j)= this%Meshnum(2)/nproccol
       enddo
    enddo

    this%Sptrnglocal(1) = xmin
    this%Sptrnglocal(2) = xmax
    this%Sptrnglocal(3) = ymin + myidy*(ymax-ymin)/nproccol
    this%Sptrnglocal(4) = ymin + (myidy+1)*(ymax-ymin)/nproccol
    this%SpatRange(1) = xmin
    this%SpatRange(2) = xmax
    this%SpatRange(3) = ymin
    this%SpatRange(4) = ymax
  ...
  end subroutine update2dold_CompDom



  !update geometry parameters using new particle positions.
  subroutine update2d_CompDom(this,ptrange1,grid2d)
    implicit none
    include 'mpif.h'
    type (CompDom), intent(inout) :: this
    type (Pgrid2d), intent(in) :: grid2d
    double precision, dimension(4), intent(in) :: ptrange1
    double precision, dimension(2) :: localrange1, &
         localrange2,temp1,temp2
    integer :: i,j,comm2d,commrow,commcol,nproc,nproccol,nprocrow,&
         myid,myidx,myidy,nphalf,npbc
    integer :: ierr
    double precision :: xmin,xmax,ymin,ymax,hx,hy,deltay,&
         epsx,epsy
    double precision :: t0
    ...
    do i = 0, nprocrow-1
       do j = 0, nphalf-1
          this%LcTabnm(2,i,j)= this%Meshnum(2)/nphalf
       enddo
    enddo

    this%Sptrnglocal(1) = xmin
    this%Sptrnglocal(2) = xmax
    this%Sptrnglocal(3) = ymin + (myidy-npbc)*(ymax-ymin)/nphalf
    this%Sptrnglocal(4) = ymin + (myidy+1-npbc)*(ymax-ymin)/nphalf
    this%SpatRange(1) = xmin
    this%SpatRange(2) = xmax
    this%SpatRange(3) = ymin
    this%SpatRange(4) = ymax
  ...
  end subroutine update2d_CompDom



  !update geometry parameters using new particle positions.
  subroutine update2dglb_CompDom(this,ptrange1,grid2d)
    implicit none
    include 'mpif.h'
    type (CompDom), intent(inout) :: this
    type (Pgrid2d), intent(in) :: grid2d
  ...
  end subroutine update2dglb_CompDom



  !--------------------------------------------------------------------------
  subroutine construct1G_CompDom(this,distparam,nparam, &
       flg,nx,ny,nz,grid2d,nprocrow,nproccol,Flagbc,xrad,yrad,perd)
    implicit none
    include 'mpif.h'
    type (Pgrid2d), intent(in) :: grid2d
  ...
  end subroutine construct1G_CompDom



  !update geometry parameters using new particle positions and
  !1 group PEs.
  subroutine update2d1G_CompDom(this,ptrange1,ptrange2,grid2d)
    implicit none
    include 'mpif.h'
    type (CompDom), intent(inout) :: this
    type (Pgrid2d), intent(in) :: grid2d
    ...
    do i = 1, 2
       if(abs(ptrange1(2*i-1)).gt.abs(ptrange2(2*i-1))) then
          localrange1(i) = ptrange1(2*i-1)
       else
          localrange1(i) = ptrange2(2*i-1)
       endif
       if(abs(ptrange1(2*i)).gt.abs(ptrange2(2*i))) then
          localrange2(i) = ptrange1(2*i)
       else
          localrange2(i) = ptrange2(2*i)
       endif
  ...
  end subroutine update2d1G_CompDom
